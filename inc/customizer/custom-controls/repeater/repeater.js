...
jQuery(document).ready(function($) {
    ...
    function blogisticClickOutSideElm(elm, callback) {
    ...
    $( ".blogistic-repeater-control" ).each(function() {
        var container = $(this)
        ...
        container.on( "click", ".image-field .add-image-trigger", function(event) {
            ...
            frame.on('select', function() {
                ...
                _this.parents( ".blogistic-repeater-item" ).addClass("popupActive")
        ...
        container.on( "click", ".fontawesome-icon-picker .icons-list i", function() {
            ...
            if( ! _this.hasClass('selected') ) {
                ...
                _this.parents( ".blogistic-repeater-item" ).find( ".item-heading-wrap .item-heading" ).text(newValue.split("-")[1])
            }
            repeater_value_refresh(_this);
        })

        // sortable handler
        container.find( ".blogistic-repeater-control-inner" ).sortable({
            orientation: "vertical",
            items: "> .blogistic-repeater-item",
            update: function (event, ui) {
                $(this).find( " > .blogistic-repeater-item .remove-item" ).show()
                $(this).find( ".blogistic-repeater-item:first .remove-item" ).hide()
        ...
        container.on( "click", ".display-icon", function() {
            ...
            _this.parents( ".blogistic-repeater-item" ).toggleClass( "not-visible visible" )
            // _this.parent().next().toggleClass("isShow isHidden");
            // _this.parents( ".blogistic-repeater-item" ).toggleClass("popupActive");
            repeater_value_refresh(_this)
        })
        ...
        container.on( "click", ".remove-item", function(e) {
            e.preventDefault();
            var controller = $(this).parents( ".blogistic-repeater-control" ), toRemove = $(this).parents( ".blogistic-repeater-item" );
        ...
        container.on( "click", ".add-new-item", function(e) {
            ...
            blogisticClickOutSideElm( fieldsPopup, function() {
                fieldsPopup.parents( ".blogistic-repeater-item" ).removeClass("popupActive")
        ...
        container.on( "click", ".blogistic-repeater-item.visible .settings-icon", function() {
            $(this).parents(".blogistic-repeater-item").siblings().removeClass("popupActive").find(".item-control-fields").removeClass("isShow").addClass("isHidden")
            $(this).parents(".blogistic-repeater-item").siblings().find( ".fontawesome-icon-picker .icons-list" ).hide()
            $(this).parents(".blogistic-repeater-item").toggleClass("popupActive")
            $(this).parent().next().toggleClass( "isHidden isShow" );
            $(this).parent().next().find( ".fontawesome-icon-picker .icons-list" ).hide();
            // close popup on outside click
            var fieldsPopup =  $(this).parents(".blogistic-repeater-item").find( ".item-control-fields.isShow" )
            blogisticClickOutSideElm( fieldsPopup, function() {
                fieldsPopup.parents( ".blogistic-repeater-item" ).removeClass("popupActive")
                fieldsPopup.removeClass( "isShow" ).addClass( "isHidden" )
            })
        ...
        container.on( "change keyup", ".blogistic-repeater-item .repeater-field-value-holder", function() {
            var _this = $(this)
            repeater_value_refresh(_this)
        })
    ...
    function repeater_value_refresh( _this ) {
        var controlValue = [], container =  ( _this.hasClass( "blogistic-repeater-control" ) ) ? _this : _this.parents( ".blogistic-repeater-control" );
        container.find( ".blogistic-repeater-item" ).each(function() {
            var newValue = {}
